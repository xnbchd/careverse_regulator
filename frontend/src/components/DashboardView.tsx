        ...
        <CardContent className="p-6">
          <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6 mb-6">
            ...
            <div className="flex flex-wrap gap-2">
              <Button onClick={() => onNavigate('license-management')}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            >
              <span className="text-sm text-muted-foreground mb-1 truncate w-full">Pending renewals</span>
              <strong className="text-2xl font-bold mb-1">{activeBoard?.pendingRenewals || '--'}</strong>
              <em className="text-xs text-muted-foreground not-italic truncate w-full">{activeBoard?.unitName || 'Active company unit'}</em>
            </button>
            <button
              type="button"
              className="flex flex-col items-start p-4 rounded-lg border border-border bg-background hover:bg-accent transition-colors text-left min-w-0"
            ...
            >
              <span className="text-sm text-muted-foreground mb-1 truncate w-full">Active cases</span>
              <strong className="text-2xl font-bold mb-1">{activeBoard?.activeCases || '--'}</strong>
              <em className="text-xs text-muted-foreground not-italic truncate w-full">Enforcement and discipline</em>
            </button>
            <button
              type="button"
              className="flex flex-col items-start p-4 rounded-lg border border-border bg-background hover:bg-accent transition-colors text-left min-w-0"
            ...
            >
              <span className="text-sm text-muted-foreground mb-1 truncate w-full">SLA compliance</span>
              <strong className="text-2xl font-bold mb-1">{activeBoard ? `${activeBoard.slaCompliance}%` : '--'}</strong>
              <em className="text-xs text-muted-foreground not-italic truncate w-full">Operational throughput</em>
            </button>
          </div>
        </CardContent>
      </Card>
              ...
              <CardContent className="space-y-2">
                {visibleQueues.length > 0 ? (
                  visibleQueues.map((queue) => (
                    <button
                      ...
                      className={cn(
                        ...
                        queue.tone === 'critical' && "border-red-500/50 bg-red-50 dark:bg-red-950/20 hover:bg-red-100 dark:hover:bg-red-950/30",
                        queue.tone === 'attention' && "border-yellow-500/50 bg-yellow-50 dark:bg-yellow-950/20 hover:bg-yellow-100 dark:hover:bg-yellow-950/30",
                  ...
                  <div className="p-4 rounded-lg border border-border bg-background">
                    <p className="font-semibold text-sm mb-1">No queue items in this company context</p>
                    <p className="text-xs text-muted-foreground">Once applications are loaded for this company, queue items will appear here.</p>
                  </div>
                )}
              </CardContent>
            </Card>
                  ...
                  visibleCountyRisk.map((item) => (
                    <button
                      ...
                      className={cn(
                        ...
                        item.riskLevel === 'Critical' && "border-red-500/50 bg-red-50 dark:bg-red-950/20 hover:bg-red-100 dark:hover:bg-red-950/30",
                        item.riskLevel === 'High' && "border-orange-500/50 bg-orange-50 dark:bg-orange-950/20 hover:bg-orange-100 dark:hover:bg-orange-950/30",
                        item.riskLevel === 'Moderate' && "border-yellow-500/50 bg-yellow-50 dark:bg-yellow-950/20 hover:bg-yellow-100 dark:hover:bg-yellow-950/30",
                        item.riskLevel === 'Stable' && "border-green-500/50 bg-green-50 dark:bg-green-950/20 hover:bg-green-100 dark:hover:bg-green-950/30"
                      )}
                      onClick={() => onNavigate(item.route)}
                    >
                      <div className="flex items-center justify-between gap-2">
                        <p className="font-semibold text-sm truncate min-w-0 flex-1">{item.county}</p>
                        <span className="text-xs font-medium px-2 py-0.5 rounded-full bg-background/50 shrink-0">{item.riskLevel}</span>
                      </div>
                      <p className="text-xs text-muted-foreground truncate">{item.openInvestigations}</p>
                      <Progress value={item.complianceRate} className="h-1.5" />
                    </button>
                  ))
                ) : (
                  <div className="p-4 rounded-lg border border-border bg-background">
                    <p className="font-semibold text-sm mb-1">No county risk items for this company</p>
                    <p className="text-xs text-muted-foreground">Risk watch will populate once company data is linked.</p>
                  </div>
                )}
              </CardContent>
            </Card>
          ...
          <Card className="bg-card/50 backdrop-blur-sm">
            ...
            <CardContent className="space-y-2">
              {visibleActivity.length > 0 ? (
                ...
                <div className="p-4 rounded-lg border border-border bg-background">
                  <p className="font-semibold text-sm mb-1">No recent activity in this company context</p>
                  <p className="text-xs text-muted-foreground">Operational events will appear here once this company starts processing cases.</p>
                </div>
              )}
            </CardContent>
          </Card>
