...
export default function FacilityApplicationPage({ application: initialApp }: FacilityApplicationPageProps) {
  const navigate = useNavigate()
  const [application, setApplication] = useState(initialApp)
  const [remarks, setRemarks] = useState('')
  const [submitting, setSubmitting] = useState<string | null>(null)
  const [selectedAction, setSelectedAction] = useState<'Issued' | 'Denied' | 'Info Requested' | null>(null)
  const decisionRef = useRef<HTMLDivElement>(null)

  const isPending = application.applicationStatus === 'Pending' || application.applicationStatus === 'Info Requested'
  ...
  const handleSubmitAction = async () => {
    ...
    try {
      await updateFacilityApplicationStatus(application.licenseApplicationId, status as 'Issued' | 'Denied', remarks)
      showSuccess(`Application ${selectedAction === 'Issued' ? 'issued' : selectedAction === 'Denied' ? 'denied' : 'info requested'} successfully`)
      useLicensingStore.getState().fetchApplications()
      setApplication({ ...application, applicationStatus: selectedAction === 'Info Requested' ? 'Info Requested' : status as any })
      setRemarks('')
  ...
  const actionButtonLabel = selectedAction === 'Issued' ? 'Issue License' : selectedAction === 'Denied' ? 'Deny Application' : 'Request Info'
        ...
        <Card>
          ...
          <CardContent>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <ReadOnlyField label="Phone Number" value={application.telephoneNumber} icon={Phone} />
              ...
              <ReadOnlyField label="Number of Beds" value={application.numberOfBeds?.toString()} icon={Bed} />
              <ReadOnlyField label="Open Whole Day" value={application.openWholeDay ? 'Yes' : 'No'} icon={Clock} />
              <ReadOnlyField label="Open Public Holiday" value={application.openPublicHoliday ? 'Yes' : 'No'} />
              <ReadOnlyField label="Open Weekends" value={application.openWeekends ? 'Yes' : 'No'} />
              <ReadOnlyField label="Open Late Night" value={application.openLateNight ? 'Yes' : 'No'} />
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <Card className="mt-5">
        ...
        <CardContent className="space-y-3">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            <ReadOnlyField label="Fee Paid" value={application.licenseFee ? `KES ${application.licenseFee.toLocaleString()}` : undefined} icon={DollarSign} />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 mt-1.5">
                {application.complianceDocuments.map((doc: any, idx: number) => (
                  <div key={idx} className="flex items-center gap-3 p-2.5 bg-muted/50 rounded-lg border border-border">
                    ...
                    <span className="text-sm flex-1 truncate">{doc.document_type || doc.name || `Document ${idx + 1}`}</span>
                  </div>
                ))}
              </div>
            </div>
