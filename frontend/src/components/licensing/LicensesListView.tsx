...
export default function LicensesListView({ company }: LicensesListViewProps) {
  ...
  const [profLicSearchText, setProfLicSearchText] = useState(professionalLicensesFilters.search || '')
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      setProfessionalLicensesFilters({ ...professionalLicensesFilters, search: profLicSearchText || undefined })
    }, 300)
    return () => clearTimeout(timer)
  }, [profLicSearchText])
  ...
  const handleProfLicSortChange = (order: 'asc' | 'desc' | 'recent') => {
    ...
    if (order === 'asc') {
      setProfessionalLicensesFilters({ ...professionalLicensesFilters, sortBy: 'license_number', sortOrder: 'asc' })
    } else if (order === 'desc') {
      setProfessionalLicensesFilters({ ...professionalLicensesFilters, sortBy: 'license_number', sortOrder: 'desc' })
    } else {
      setProfessionalLicensesFilters({ ...professionalLicensesFilters, sortBy: 'expiry_date', sortOrder: 'desc' })
    }
  }
  ...
  const handleApplySavedLicenseFilters = (filters: LicenseFiltersState, setSearch: (v: string) => void, setStatuses: (v: string[]) => void, setSort: (v: 'asc' | 'desc' | 'recent') => void, setStoreFilters: (f: any) => void) => {
    setSearch(filters.search || '')
    setStatuses(filters.status || ['all'])
    setSort(filters.sortOrder || 'recent')
    const statusFilter = filters.status?.join(',')
    if (filters.sortOrder === 'asc') {
      setStoreFilters({ search: filters.search, status: statusFilter, sortBy: 'license_number', sortOrder: 'asc' })
    } else if (filters.sortOrder === 'desc') {
      setStoreFilters({ search: filters.search, status: statusFilter, sortBy: 'license_number', sortOrder: 'desc' })
    } else {
      setStoreFilters({ search: filters.search, status: statusFilter, sortBy: 'expiry_date', sortOrder: 'desc' })
    }
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as TabValue)} className="w-full">
        ...
        <TabsContent value="facility-licenses" className="space-y-6">
          <div className="flex justify-between items-start gap-4 flex-wrap">
            ...
            <div className="flex gap-2">
              <SavedFiltersManager
                ...
                onApplyFilters={(f) => handleApplySavedLicenseFilters(f, setFacilitySearchText, setSelectedFacilityStatuses, setFacilitySortOrder, setLicensesFilters)}
          ...
          {isMobile || isTablet ? (
            <div className="space-y-4">
              {licenses.map((license) => (
                <LicenseCard key={license.id} license={license} onClick={() => handleLicenseRowClick(license.licenseNumber)} />
              ))}
            </div>
          ) : (
            <LicensesTable
              ...
              emptyState={<EmptyStateInline icon={Building2} title="No Facility Licenses Found" description={activeFacilityLicFilters > 0 ? 'No facility licenses match your current filters.' : 'There are currently no health facility licenses in the system.'} onClear={activeFacilityLicFilters > 0 ? handleClearFacilityFilters : undefined} />}
            />
          )}
          ...
          <BulkActionsBar
            ...
            actions={[
              { label: 'Approve', onClick: () => handleBulkAction('APPROVE'), variant: 'default', icon: <CheckCircle className="w-4 h-4" />, loading: bulkLicenseActionLoading },
              { label: 'Deny', onClick: () => handleBulkAction('DENY'), variant: 'destructive', icon: <XCircle className="w-4 h-4" />, loading: bulkLicenseActionLoading },
              { label: 'Suspend', onClick: () => handleBulkAction('SUSPEND'), variant: 'secondary', icon: <Ban className="w-4 h-4" />, loading: bulkLicenseActionLoading },
            ]}
          />
        ...
        <TabsContent value="professional-licenses" className="space-y-6">
          <div className="flex justify-between items-start gap-4 flex-wrap">
            ...
            <div className="flex gap-2">
              <SavedFiltersManager
                ...
                onApplyFilters={(f) => handleApplySavedLicenseFilters(f, setProfLicSearchText, setSelectedProfLicStatuses, setProfLicSortOrder, setProfessionalLicensesFilters)}
                getFilterSummary={getLicenseFilterSummary}
              />
              <ExportButton data={professionalLicenses} config={profLicenseExportConfig} size="default" />
            </div>
          </div>
          ...
          <ProfessionalLicensesTable
            ...
            onToggleSelection={(id) => toggleSelection(selectedProfLicIds, setSelectedProfLicIds, id)}
            onSelectAll={() => setSelectedProfLicIds(new Set(professionalLicenses.map(l => l.licenseNumber)))}
      ...
      <BulkActionConfirmDialog
        ...
        onClose={() => { setShowBulkActionDialog(false); setBulkActionType(null) }}
        title={`${bulkActionType ? getBulkActionLabel(bulkActionType) : ''} Selected Licenses`}
        description={`You are about to ${bulkActionType ? getBulkActionLabel(bulkActionType).toLowerCase() : 'update'} the selected licenses.`}
        selectedCount={selectedLicenseIds.size}
