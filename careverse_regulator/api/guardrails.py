...
def with_company_filter(
        filters: dict[str, Any] | None = None, user: str | None = None) -> dict[str, Any]:
    ...
    if requested_company and str(requested_company).strip() != company:
        frappe.throw(
            _("Cross-company filters are not allowed."),
            frappe.PermissionError)
...
def set_doc_company(doc: Any, user: str | None = None) -> None:
    ...
    if doc_company and str(doc_company).strip() != company:
        frappe.throw(
            _("Company mismatch in request payload."),
            frappe.PermissionError)
...
def has_company_permission(
        doc: Any,
        user: str | None = None,
        permission_type: str | None = None) -> bool | None:
