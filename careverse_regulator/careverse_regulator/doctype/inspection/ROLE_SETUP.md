# Inspection DocType - Role Setup Guide

## Overview

The Inspection DocType requires proper role and permission configuration for multi-tenant operation. This document outlines the roles and User Permissions needed for deployment.

## Required Roles

The Inspection DocType uses three pre-defined roles with different permission levels:

### 1. Regulator Admin
- **Permissions**: Full access (create, read, write, delete, export, import, share)
- **Use Case**: System administrators who manage inspections across all companies
- **Special Abilities**:
  - Can reassign inspections between companies
  - Can delete inspections
  - Can modify any inspection regardless of ownership

### 2. Inspector
- **Permissions**: Create, read, write (limited)
- **Use Case**: Field inspectors who create and complete inspections
- **Limitations**:
  - Can only edit inspections they own (if_owner=1)
...
- **Use Case**: Staff who need to view inspection data
- **Limitations**:
  - Cannot create, edit, or delete inspections
  - Scoped to their assigned company via User Permissions
...
3. **Verify Access**
   ...
   - System will auto-assign company to new inspections
   - User can only see inspections from their assigned company
...
- **Automatic Isolation**: The permission query function automatically filters inspections by company at the database level.
- **Company Auto-Assignment**: When creating inspections, the company field is automatically populated from the user's User Permission.
...
# 3. Test creating an inspection as this user
# 4. Verify company is auto-assigned
# 5. Verify user can only see their company's inspections
```

### Validation Checklist

- [ ] User has exactly one Company User Permission
- [ ] User can create inspections without manually setting company
- [ ] Company field is automatically populated
- [ ] User cannot see inspections from other companies
- [ ] Regulator Admin can reassign inspections between companies
...
- **DocType Path**: `apps/careverse_regulator/careverse_regulator/careverse_regulator/doctype/inspection/`
- **Permission Query**: `inspection.get_permission_query_conditions()`
