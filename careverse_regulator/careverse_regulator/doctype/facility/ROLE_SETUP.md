# Facility DocType - Role Setup Guide

## Overview

The Facility DocType requires proper role and permission configuration for multi-tenant operation. This document outlines the roles and User Permissions needed for deployment.

## Required Roles

The Facility DocType uses three pre-defined roles with different permission levels:

### 1. Regulator Admin
- **Permissions**: Full access (create, read, write, delete, export, import, share)
- **Use Case**: System administrators who manage facilities across all companies
- **Special Abilities**:
  - Can reassign facilities between companies
  - Can delete facilities
  - Can modify any facility regardless of ownership

### 2. Inspector
- **Permissions**: Create, read, write (limited)
- **Use Case**: Field inspectors who create and manage facility records
- **Limitations**:
  - Can only edit facilities they own (if_owner=1)
...
- **Use Case**: Staff who need to view facility data
- **Limitations**:
  - Cannot create, edit, or delete facilities
  - Scoped to their assigned company via User Permissions
...
3. **Verify Access**
   ...
   - System will auto-assign company to new facilities
   - User can only see facilities from their assigned company
...
- **Automatic Isolation**: The permission query function automatically filters facilities by company at the database level.
- **Company Auto-Assignment**: When creating facilities, the company field is automatically populated from the user's User Permission.
...
# 3. Test creating a facility as this user
# 4. Verify company is auto-assigned
# 5. Verify user can only see their company's facilities
```

### Validation Checklist

- [ ] User has exactly one Company User Permission
- [ ] User can create facilities without manually setting company
- [ ] Company field is automatically populated
- [ ] User cannot see facilities from other companies
- [ ] Regulator Admin can reassign facilities between companies
- [ ] Inspectors can only edit their own facilities
...
- **DocType Path**: `apps/careverse_regulator/careverse_regulator/careverse_regulator/doctype/facility/`
- **Permission Query**: `facility.get_permission_query_conditions()`
