"""
Facility DocType - Manages facility master data

This DocType stores facility records for regulatory oversight.
Each facility is scoped to a single Company for multi-tenant isolation.
...
class Facility(Document):
	"""Facility document controller"""

	def before_insert(self) -> None:
		"""Auto-assign company from user context before saving new facility"""
		...
		try:
			...
			frappe.throw(
				frappe._("Cannot create facility: {0}").format(str(e)),
...
def get_permission_query_conditions(user: str) -> str:
	...
	return f"`tabFacility`.`company` IN ({company_list})"
